...
if [ "$CHECK" = true ]; then
  ...
  determine_exit_code $?
  # --> GatorGrader CHECK: at least 6 "println()" fragment exists in the code
  python3 gatorgrader/gatorgrader.py --nowelcome --directories src/main/java/practicalone \
                                     --checkfiles Swap.java --fragments "println(" --fragmentcounts 6
  determine_exit_code $?
  # --> GatorGrader CHECK: at least 6 "println()" fragment exists in the code
  python3 gatorgrader/gatorgrader.py --nowelcome --directories src/test/java/practicalone \
                                     --checkfiles TestSwap.java --fragments "@Test" --fragmentcounts 3
  determine_exit_code $?
  # --> GatorGrader CHECK: running the program produces exactly 6 lines of output
